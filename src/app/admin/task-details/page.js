...
export default function TaskDetailsPage() {
  ...
  const filteredTasks = !companySearch.trim()
    ? tasks
    : tasks.filter((t) =>
        (t.companyName || '')
          .toLowerCase()
      ...
      <div className="pt-16 md:pl-64 transition-all duration-300 min-h-screen">
        <div className="max-w-full mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          {!loading && tasks.length > 0 && (
            <div className="relative mb-5">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3  md:w-1/2 rounded-2xl border border-slate-200 bg-white shadow-sm text-sm text-slate-700 font-medium placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-300 transition-all"
              />
              {companySearch && (
          ...
          {loading ? (
            ...
            <div className="text-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm">
              <p className="text-slate-500 font-medium">No tasks matching company &quot;{companySearch}&quot;.</p>
              <button type="button" onClick={() => setCompanySearch('')}
                className="mt-3 text-sm font-semibold text-violet-600 hover:text-violet-800">Clear search</button>
            </div>
          ) : viewMode === 'table' ? (
            /* ── TABLE VIEW ── */
            <>
