...
export default function Navbar({ user, onLogout }) {
  ...
  return (
    <>
      ...
      >
        {/* Logo / Firm branding */}
        <div className={`flex items-center gap-3 px-5 py-6 border-b border-white/10 ${sidebarCollapsed ? 'justify-center px-3' : ''}`}>
          <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg overflow-hidden bg-gradient-to-br ${roleGradient}`}>
            {user?.firm?.logo ? (
              <img src={user.firm.logo} alt="" className="w-full h-full object-contain" />
            ) : (
        ...
        <div className={`p-3 border-t border-white/10`}>
          <div
            className={`relative flex items-center gap-3 p-3 rounded-xl hover:bg-white/10 cursor-pointer transition-all ${sidebarCollapsed ? 'justify-center' : ''}`}
          ...
          >
            ...
            {/* Dropdown */}
            {showMenu && (
              <div className={`absolute bottom-full mb-2 bg-white rounded-2xl shadow-2xl border border-slate-100 w-56 overflow-hidden z-50 ${sidebarCollapsed ? 'left-full ml-2 bottom-0' : 'left-0'}`}>
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 truncate">
            <span className="font-medium text-slate-800">
              {user?.role === 'superadmin' ? 'Super Admin' : user?.role === 'admin' ? 'Admin' : user?.role === 'manager' ? 'Manager' : 'Employee'}
            </span>
            <span>/</span>
